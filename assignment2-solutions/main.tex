...
\begin{enumerate}
    ...
    $$F(y) = \text{ choose an element from the set } \{x : f(x) = y \}$$ 
    We can choose these elements as a direct consequence of the Axiom of Choice. $F$ can be shown to be a injection, since $f$ is a function. Then, use the results in Problem 22.
    ...
    \item Show that the sets $[0, 1]$ and $\p(\nn)$ are equipotent.
	    ...
	    \begin{cases}
		    \phi & \text{ if } x = 0 \\
		    \{\left \lfloor 10^n(x)_2 \right \rfloor : n \in \nn \} & \text{ if } x \neq 0
	    \end{cases}
	    \end{equation*}
